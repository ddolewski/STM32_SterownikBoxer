...
void MainTimer_Handle(void)
{
	if (systimeTimeoutControl(&oneSecTimer, 1000))
	{
		...
		/*							CONTROLLING LAMP                            */
		/************************************************************************/
		lastLightState = lightControl.lightingState;
		if (lightControl.timeOnHours == 0 && lightControl.timeOffHours == 24)
		{
			GPIOx_ResetPin(LAMP_PORT, LAMP_PIN);
			lightControl.lightingState = LIGHT_OFF;
			lightCounters.counterSeconds = 0;
		}
		else if (lightControl.timeOnHours == 24 && lightControl.timeOffHours == 0)
		{
			GPIOx_ResetPin(LAMP_PORT, LAMP_PIN);
			lightControl.lightingState = LIGHT_ON;
			lightCounters.counterSeconds = 0;
		}
		else
		{
			lightCounters.counterSeconds++;
			if (lightCounters.counterSeconds == 3600)
			{
				lightCounters.counterHours++;
				lightCounters.counterSeconds = 0;
			}

			if (lightControl.lightingState == LIGHT_ON) //lampa wlaczona
			{
				if (lightControl.timeOnHours != 0 && lightControl.timeOnHours != 24)
				{
					GPIOx_SetPin(LAMP_PORT, LAMP_PIN);
					if (lightControl.timeOnHours == lightCounters.counterHours)
					{
						lightControl.lightingState = LIGHT_OFF;
			...
			else if (lightControl.lightingState == LIGHT_OFF) //lampa wylaczona
			{
				if (lightControl.timeOffHours != 0 && lightControl.timeOffHours != 24)
				{
					GPIOx_ResetPin(LAMP_PORT, LAMP_PIN);
					if (lightControl.timeOffHours == lightCounters.counterHours)
					{
						lightControl.lightingState = LIGHT_ON;
						lightCounters.counterHours = 0;
						lightCounters.counterSeconds = 0;
